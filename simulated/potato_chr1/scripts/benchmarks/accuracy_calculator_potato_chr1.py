...
def loadBenchmarkParameterFile(parameterFilePath):
    ...
    for line in benchmarkParameterFile:
        ...
        # elif line[0] in {"lognormalMean","lognormalStd"}:
        #     for dataPoint in line[1:]:
        #         dataPoint = dataPoint.split(",")
        #         benchmarkParameters[line[0]].append(dataPoint)
...
def getTruePhases(truthTxtPath):
    referenceDict={}
    truePhaseDict={}
    heterozygosityChecker={}
    sampleNames=[]
    with open(truthTxtPath,"r") as truthTxt:
        for line in truthTxt:
            line=line.strip("\n").split("\t")
            if line[0][0]=="B":
                sampleNames = line[6:]
            else:
                ...
                for i in range(len(haps)):
                    if haps[i] == "0":
                        ...
                        SNP = SNPs[int(haps[i])-1]
                    fullSNP=chromosome+":"+SNPPosition+"="+SNP
                    truePhaseDict.setdefault(chromosome,{})
                    truePhaseDict[chromosome].setdefault(SNPPosition, {})
                    truePhaseDict[chromosome][SNPPosition].setdefault(sampleNames[i],set())
                    truePhaseDict[chromosome][SNPPosition][sampleNames[i]].add(fullSNP)
...
def calculateAccuracyMetrics(groundTruthInfo,predictedInfo,PhasingReadsResults,ReadsHaplotypes,testInfo,accuracyMetricFile):
        ...
        for predictionName, SNPs in haplotypes.items():
            ...
            if predictionName in PhasingReadsResults:
                ...
                for readName in readNameSet:
                    ...
                    readHapChr = ReadsHaplotypes[readName].split("#")
                    if readHapChr[0] == closestHaplotypeName and readHapChr[1] == chromosome:
...
def subsetTruePhases(groundTruthDict,subsetFile,hybridVCFPrefix):
    VCFInfo=loadVCFInformation(os.path.join(hybridVCFPrefix,subsetFile))
...
def loadReadsHaplotypes(longReadsFile):
    readsHaplotypes = dict()
    gz = gzip.open(longReadsFile, "rb")
    ...
    for num, line in enumerate(handle):
        ...
        if num % 4 == 0:
            if line[0:1] != b"@":
                raise FastaError("Fastq format error: {0} at line {1}".format(handle.name, num))
            line = line[1:].split()
            read_name = line[0].decode()
            haplotype_name = line[1].split(b",")[0].decode()
            # hap_1#CP048984.1
            readsHaplotypes[read_name] = haplotype_name

    return readsHaplotypes
...
if __name__ == "__main__":
    ...
    GTPrefix=os.path.join(mainPath, "groundTruth")
    ...
    hybridVCFPrefix=os.path.join(mainPath, "virtualPolyploids/Variants_filter/")
        ...
        for index in range(len(Ploidy)):
            ...
            for coverage in coverages:
                for longReadMethod in longReadMethods:
                    ...
                    truePhaseGenotypes=[]
                    truthFileName = virtualName+"_varianthaplos.txt"
                    truthFile = os.path.join(GTPrefix, virtualName, truthFileName)
                    truePhaseDict[virtualName] = getTruePhases(truthFile)

                    # subsetFileName=virtualName+".vcf"
                    subsetFileName=testName+"_lofreq_filter.vcf"
                    # subsetFileName=virtualName+".vcf"
                    allSubsets[subsetFileName]=subsetTruePhases(truePhaseDict[virtualName],subsetFileName,hybridVCFPrefix)

            for coverage in coverages:
                for longReadMethod in longReadMethods:
                    testName = virtualName + "_" + str(coverage) + "X_"+longReadMethod
                    longReadsFile = testName +".fastq.gz"
                    longReadsPath = os.path.join(mainPath, "virtualPolyploids/rawReads/longReads", longReadsFile)
                    AllReadsHaplotypes[longReadsFile] = loadReadsHaplotypes(longReadsPath)
            ...
            for index in range(len(Ploidy)):
                ...
                for coverage in coverages:
                    for longReadMethod in longReadMethods:
                        testName = virtualName + "_" + str(coverage) + "X_"+longReadMethod
                        subsetFileName=testName+"_lofreq_filter.vcf"
                        mappedLongFile = os.path.join(hybridLongMapPrefix, testName + ".sorted.bam")
                        whatsHapPolyphaseParameter = "4"
                        phasingMethod="WHP-PP_B"+whatsHapPolyphaseParameter
                        testInfo=[phasingMethod,ploidy,mut,coverage]
                        VCFFileName = testName+".vcf"
                        ...
                        phasedBam = os.path.join(accuracyCalculationPath, "Translated_WHP_"+testName+".sorted.bam")
                        TranslatedClusteredReadsFile = os.path.join(accuracyCalculationPath, "Translated_WHP_"+testName+"_clustered_read_name.tsv")
                        ...
                        longReadsFile = testName +".fastq.gz"
                        ReadsHaplotypes = AllReadsHaplotypes[longReadsFile]
                        calculateAccuracyMetrics(allSubsets[subsetFileName],WHPPhasingResults,WHPPhasingReadsResults,ReadsHaplotypes,testInfo,accuracyMetricFile)
            ...
            for index in range(len(Ploidy)):
                ...
                for coverage in coverages:
                    for longReadMethod in longReadMethods:
                        ...
                        VCFFileName = testName+".vcf"
                        testInfo=[phasingMethod,ploidy,mut,coverage]
                        VCFInformation=loadVCFInformation(os.path.join(hybridVCFPrefix,subsetFileName))
                        ...
                        FloppClusteredReadsPath = os.path.join(floppPrefix,"flopp_"+testName)
                        FloppClusteredReadsFiles = glob.glob(os.path.join(FloppClusteredReadsPath, "*_part.txt"))
                        floppReadsResults = loadFloppReadsResults(FloppClusteredReadsFiles)
                        longReadsFile = testName +".fastq.gz"
                        ReadsHaplotypes = AllReadsHaplotypes[longReadsFile]
                        calculateAccuracyMetrics(allSubsets[subsetFileName], floppPredictions,floppReadsResults,ReadsHaplotypes,testInfo,accuracyMetricFile)
            ...
            for index in range(len(Ploidy)):
                ...
                for coverage in coverages:
                    for longReadMethod in longReadMethods:
                        ...
                        subsetFileName=testName+"_lofreq_filter.vcf"
                        mappedLongFile = os.path.join(hybridLongMapPrefix, testName + ".sorted.bam")
                        VCFFileName = testName+nPhaseDefaultSuffix
                        phasingMethod="nPhase"
                        testInfo=[phasingMethod,ploidy,mut,coverage]
                        nPhaseResultPath = os.path.join(nPhasePrefix, testName, "Phased/", VCFFileName)
                        nPhaseReadsResultPath = os.path.join(nPhasePrefix, testName, "Phased/", testName+nPhaseDefaultReadsSuffix)
                        ...
                        longReadsFile = testName +".fastq.gz"
                        ReadsHaplotypes = AllReadsHaplotypes[longReadsFile]
                        calculateAccuracyMetrics(allSubsets[subsetFileName],nPhasePhasingResults,nPhasePhasingReadsResults,ReadsHaplotypes,testInfo,accuracyMetricFile)
        ...
        for mut in mutations:
            for index in range(len(Ploidy)):
                ...
                for coverage in coverages:
                    for longReadMethod in longReadMethods:
                        testName = virtualName + "_" + str(coverage) + "X_"+longReadMethod
                        """ if run ./phased_tool_runners_potato_chr1.py, 
                        ...
                        testInfo=[phasingMethod,ploidy,mut,coverage]
                        PhasingResultPath = os.path.join(PhasingPrefix, testName, "Phased/", VCFFileName)
                        PhasingReadsResultPath = os.path.join(PhasingPrefix, testName, "Phased/", "phased"+PhasingDefaultReadsSuffix)
                        subsetFileName=testName+"_lofreq_filter.vcf"
                        PhasingResults = loadPhasingResults(PhasingResultPath)
                        PhasingReadsResults = loadClusteredReadName(PhasingReadsResultPath)
                        longReadsFile = testName +".fastq.gz"
                        ReadsHaplotypes = AllReadsHaplotypes[longReadsFile]
                        calculateAccuracyMetrics(allSubsets[subsetFileName],PhasingResults,PhasingReadsResults,ReadsHaplotypes,testInfo,accuracyMetricFile)
