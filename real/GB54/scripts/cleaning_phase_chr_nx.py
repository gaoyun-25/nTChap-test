...
def PhasingCleaning(PhaseResultFolder, readSNPsPath,max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len, percentKept=99, deduplicate=True):
    percentKept = percentKept / 100
    max_ID = float(max_ID)
    ...
    min_len = int(min_len)
    phased_name = "phased"+"_"+str(min_ovl)+"_"+str(min_sim)+"_"+str(max_ID)+"_"+str(min_len)+"_"+str(min_overlap)+"_"+str(min_ovl_len)
    readClusterPath = os.path.join(PhaseResultFolder, phased_name+"_clustered_read_name.tsv")
    consensusClusterPath = os.path.join(PhaseResultFolder, phased_name+"_variants.tsv")

    outPath = os.path.join(PhaseResultFolder,"Cleaned_chr")
    plotPath = os.path.join(outPath, "Plots")
    os.makedirs(outPath, exist_ok=True)
    os.makedirs(plotPath, exist_ok=True)

    if percentKept < 1.0:
        phasedSNPFileFilterPath = os.path.join(outPath, phased_name+ "_cleaned_filter_variants.tsv")
        phasedClusteFilterPath = os.path.join(outPath, phased_name+ "_cleaned_filter_clustered_read_name.tsv")
        if os.path.exists(phasedSNPFileFilterPath):
            os.remove(phasedSNPFileFilterPath)
        if os.path.exists(phasedClusteFilterPath):
            os.remove(phasedClusteFilterPath)

    if deduplicate:
        phasedSNPFileDupPath = os.path.join(outPath, phased_name+ "_cleaned_dup_variants.tsv")
        phasedClusteDupPath = os.path.join(outPath, phased_name+ "_cleaned_dup_clustered_read_name.tsv")
        if os.path.exists(phasedSNPFileDupPath):
            os.remove(phasedSNPFileDupPath)
        if os.path.exists(phasedClusteDupPath):
            os.remove(phasedClusteDupPath)

    phasedSNPFileStitchPath = os.path.join(outPath, phased_name+ "_cleaned_stitch_variants.tsv")
    phasedClusteStitchPath = os.path.join(outPath, phased_name+ "_cleaned_stitch_clustered_read_name.tsv")
    if os.path.exists(phasedSNPFileStitchPath):
        os.remove(phasedSNPFileStitchPath)
    if os.path.exists(phasedClusteStitchPath):
        os.remove(phasedClusteStitchPath)

    clusterReadsFull = readClusterReads(readClusterPath)
    print("Number of clusters before cleaning:", len(clusterReadsFull))
    readSNPs = readReadSNPs(readSNPsPath)

    clusters_consensus = load_consensus_cluster(consensusClusterPath)
    chr_cids = getChrsCids(clusters_consensus)
    num_cid = 0
    # stitchedClustersFull = dict()
    for chr,cids in chr_cids.items():
        clusterReads = dict()
        for cid in cids:
            clusterReads[cid] = clusterReadsFull[cid]
        stitchedClusters = rebuildClusters(clusterReads, readSNPs)
        print("Number of clusters before cleaning:", len(stitchedClusters))

        if percentKept < 1.0:
            stitchedClusters = filterLowCoverageClusters(stitchedClusters, percentKept)
            makeVariantsFile(stitchedClusters, phasedSNPFileFilterPath)
            makeClusterReadFile(stitchedClusters, phasedClusteFilterPath)
            print("filter后clusters数: ", len(stitchedClusters))

        if deduplicate:
            stitchedClusters = identifyGaps(stitchedClusters, readSNPs, 50)
            print("duplicate后cluster数: ", len(stitchedClusters))
            makeVariantsFile(stitchedClusters, phasedSNPFileDupPath)
            makeClusterReadFile(stitchedClusters, phasedClusteDupPath)
        num_cid+=len(stitchedClusters)

    print("Number of clusters after cleaning:", num_cid)

    print("Cleaning done! Your cleaning results are in", outPath)


def getChrsCids(clusters_consensus):
    chr_cids = dict()
    for cid,snps in clusters_consensus.items():
        for snp in snps:
            chr = snp.split(":")[0]
            chr_cids.setdefault(chr,set())
            chr_cids[chr].add(cid)
            break
...
def filter_short_snps_clusters_from_consensus(consensus, snp_cluster, min_ovl_len):
    filter_clusters = dict()
    for cid, snps in consensus.items():
        if len(snps) >= min_ovl_len:
            filter_clusters[cid] = snp_cluster[cid]

    return filter_clusters


# rebuiltClusters[cluster]["SNPs"][SNP] += 1;rebuiltClusters[cluster]["Reads"].add(read)
# 先删掉snps seq长度小于指定阈值的小clusters(孤立点或者reads数很少的clusters)
def filter_short_pos_clusters(clusters, min_ovl_len):
    filter_low_pos_clusters = dict()
    for cluster in clusters.keys():
        SNPS = clusters[cluster]["SNPs"]
        if len(SNPS) >= min_ovl_len:
            filter_low_pos_clusters[cluster] = clusters[cluster]

    return filter_low_pos_clusters
    ...
    with open(filePath, "r") as f:
        for line in f:
            if line[0] == ">":
                ...
                chr_snps = set()
                for snp in snps:
                    chr_snps.add(ctg_name+":"+snp)
                reads_dict[line[0]] = frozenset(chr_snps)
...
def iterativeStitching(clusters, maxDiscordance):
    ...
    print("Merge后clusters个数: ", len(clusters))
    ...
    for chromosome in genomeCoverage.keys():
        ...
        for gap, gapPositions in gapPositionDict.items():
            ...
            for cluster, clusterPositions in clusterPositionDict.items():
                ...
                if overlap == set() and list(clusterPositions)[0].split(":")[0] == chromosome:  # If the cluster's in the right chromosome, doesn't overlap the gap
...
if __name__ == "__main__":
    out_dir = sys.argv[1]
    readSNPsPath = sys.argv[2]
    max_ID = sys.argv[3]
    min_len = sys.argv[4] 
    min_ovl = sys.argv[5] 
    min_sim = sys.argv[6]
    min_overlap = sys.argv[7]
    min_ovl_len = sys.argv[8]
    percentKept=float(sys.argv[9])
    deduplicate=sys.argv[10]
    if deduplicate == "0":
        deduplicate = False
    if deduplicate == "1":
        deduplicate = True
    PhasingCleaning(out_dir, readSNPsPath, max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len,percentKept,deduplicate)
