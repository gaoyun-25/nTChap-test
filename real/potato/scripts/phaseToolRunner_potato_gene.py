...
def launchFunction(functionName,parameters):
    if functionName=="ntchap":
        ...
        print(systemMessage)
        updateLog(fullLogPath, outputLog)
    elif functionName=="Phasing_gene":
        outdir=parameters[0]
        phased_dir=parameters[1]
        bed_file = parameters[2]
        mappedLR = parameters[3]
        chrom = parameters[4]
        num_top = parameters[5]
        out_name = parameters[6]
        outputLog, systemMessage=launchPlotGene(outdir, phased_dir,bed_file,mappedLR,chrom,num_top,out_name)
        print(systemMessage)
        updateLog(fullLogPath, outputLog)


def launchPlotGene(outdir, phased_dir,bed_file,mappedLR,chrom,num_top,out_name):
    start = timeit.default_timer()
    outputLog = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + "\t"

    command_py = "/home/gaoyun/poly/code/phasing/process_reads/potato/dowloadData/plot_gene_20.py"
    p = subprocess.Popen(["python3", command_py, outdir, phased_dir,bed_file,mappedLR,chrom,num_top,out_name], stderr=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
    outputLog += "COMMAND: " + " ".join(["python3", command_py, outdir, phased_dir,bed_file,mappedLR,chrom,num_top,out_name]) + "\n\n"
    ...
    command = " ".join(["python3", command_py, outdir, phased_dir,bed_file,mappedLR,chrom,num_top,out_name])
...
def launchPlot(PhasingPath, testName, max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len,longReadFastQFileDir):
    start = timeit.default_timer()
    outputLog = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + "\t"

    command_py = "/home/gaoyun/poly/code/phasing/final_nTChap/process_data/real/potato/scripts/plot.py"
    p = subprocess.Popen(["python3", command_py, PhasingPath, testName, max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len,longReadFastQFileDir], stderr=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
    outputLog += "COMMAND: " + " ".join(["python3", command_py, PhasingPath, testName, max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len,longReadFastQFileDir]) + "\n\n"
    ...
    totalRunTime = stop - start
    print("This took", totalRunTime, "seconds to run.")
    command = " ".join(["python3", command_py, PhasingPath, testName, max_ID, min_len, min_ovl, min_sim, min_overlap, min_ovl_len,longReadFastQFileDir])
    performanceLine = "\t".join([str(x) for x in [totalRunTime, maxRSS_mem, maxVMS_mem, testName,command]]) + "\n"
    pFile = open(performanceMetricFile, "a")
    pFile.write(performanceLine)
    pFile.close()
...
if __name__ == "__main__":
    ...
    predictionPath=os.path.join(mainPath, "Phasing_gene/")
    ...
    variantCalledLongReads=os.path.join(mainPath, "Variants/longReads/")
    ...
    fullLogPath=os.path.join(logPath, "phasingLog_gene.txt")
    logText=""

    #Prepare performance metrics file
    performanceMetricFile=os.path.join(performanceMetricPath, "timeAndMemoryMetrics_gene.txt")
    pFile = open(performanceMetricFile, "a")
    ...
    if "ntchap" in phasingMethods:
        ...
        for short_read in shortReadsName:
            for long_read in longReadsName:
                ...
                vcfFile=os.path.join(variantCalledLongReads, short_read+".vcf")
    ...
    if "plot" in phasingMethods:
        ...
        for short_read in shortReadsName:
            for long_read in longReadsName:
                testName=speciesName+long_read
                longReadFastQFileDir = ""
                # longReadFastQFileDir = os.path.join(mainPath,"rawData/longReads")
    ...
    if "plot_gene" in phasingMethods:
        max_ID = benchmarkParameters["max_ID"]
        min_len = benchmarkParameters["min_len"] 
        min_ovl = benchmarkParameters["min_ovl"]  
        min_sim = benchmarkParameters["min_sim"] 
        ...
        for short_read in shortReadsName:
            for long_read in longReadsName:
                ...
                outdir = os.path.join(predictionPath, testName,"Phased")
                os.makedirs(outdir, exist_ok=True)
                phased_dir = os.path.join(predictionPath, testName,"Phased")
                bed_file = os.path.join(referencePath,"annotation","all_genes.bed")
                mappedLR=os.path.join(mappedLongReads, long_read+".sorted.sam")
                chrom = "chr02"
                num_top = str(200)
                launchFunction("Phasing_gene", [outdir, phased_dir,bed_file,mappedLR,chrom,num_top,phasedName])

                plot_outdir = os.path.join(outdir,"PlotsGene/")
                os.makedirs(plot_outdir, exist_ok=True)
                n = math.ceil(int(num_top)/20)
                for i in range(n):
                    consensusClusterPath = os.path.join(plot_outdir, phasedName+"_"+str(i)+"_gene_variants.tsv")
                    readClusterPath = os.path.join(plot_outdir, phasedName+"_"+str(i)+"_gene_clustered_read_name.tsv")
                    launchFunction("plot", [consensusClusterPath, readClusterPath,plot_outdir, longReadFastQFileDir,phasedName+"_"+str(i)])
